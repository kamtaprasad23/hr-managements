...
const AttendanceTracker = () => {
  ...
  const fetchAttendance = async (date) => {
    setLoading(true);
    try {
      const formattedDate = dayjs(date).format("YYYY-MM-DD"); // backend ke liye
  ...
  const attendanceSummary = [
    ...
    { title: "Present", value: employees.filter((e) => e.status === "Present").length, icon: <CheckCircle className="text-green-500" /> },
    { title: "Absent", value: employees.filter((e) => e.status === "Absent").length, icon: <Cancel className="text-red-500" /> },
    { title: "On Leave", value: employees.filter((e) => e.status === "On Leave").length, icon: <AccessTime className="text-yellow-500" /> },
  ];
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {employees.map((employee) => (
                      <tr key={employee.id} className="hover:bg-gray-400">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">{employee.avgCheckIn}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">{employee.avgCheckOut}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button onClick={() => setSelectedEmployee(employee)} className="text-blue-600 hover:text-blue-900">
        ...
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 p-4 bg-black/50">
          <div className="rounded-lg p-6 max-w-md w-full bg-white text-black">
            ...
            <div className="space-y-3">
              ...
              <p><strong>Total Attendance:</strong> {selectedEmployee.totalAttendance}</p>
            ...
            <div className="mt-6 flex justify-end">
              <button onClick={() => setSelectedEmployee(null)} className="px-4 py-2 rounded-lg hover:bg-gray-600">Close</button>
            </div>
          </div>
        </div>
      )}
